...
void load_balancer::decrement_load(const std::array<double, Ndim>& loc, double amt) {
	hpx::async<decrement_server_action>(home,hpx::get_locality_id(), loc, amt).get();
}

hpx::future<int> load_balancer::increment_load(const std::array<double, Ndim>& l, double amt) {
	return hpx::async<increment_server_action>(home, l, amt);
}
...
int load_balancer::increment_server(const std::array<double, Ndim>& pos0, double amt) {
	boost::lock_guard<hpx::lcos::local::spinlock> this_lock(lock);
	int next_proc;

	double best_score = DBL_MAX;
	double best_load = DBL_MAX;
	...
	for( int i = 0; i < procs.size(); i++ ) {
		...
		for( int j = 0; j < Ndim; j++) {
			score += pow(pos0[j] - procs[i].loc[j], 2.0);
		}
//		score = sqrt(score) / sqrt(Ndim);
		score += 2.0*((procs[i].load + 1.0) / (best_load + 1.0) - 1.0);
		if( score < best_score ) {
			next_proc = i;
			best_score = score;
		}
...
void load_balancer::decrement_server(int proc,const std::array<double, Ndim>& pos0, double amt) {
	boost::lock_guard<hpx::lcos::local::spinlock> this_lock(lock);

	for( int i = 0; i < Ndim; i++) {
		procs[proc].loc[i] *= procs[proc].load;
		procs[proc].loc[i] -= pos0[i] * amt;
	}
	procs[proc].load -= amt;
