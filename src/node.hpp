...
public:
    ...
    hpx::shared_future<void> find_neighbors() {
        ...
        typedef boost::unique_lock<hpx::lcos::local::spinlock> scoped_lock;
        scoped_lock this_lock(branch_lock);
        ...
        if (!is_branching) {
            ...
            for (child_index_type<Ndim> ci; !ci.is_end(); ci++) {
                ...
                hpx::id_type child = children[ci];
                child_array_type local_children = children;

                hpx::util::scoped_unlock<scoped_lock> ul(this_lock);
                c2futs[ci] = hpx::async<action_note_sibs>(child, std::move(local_children));
            ...
            auto f = when_all(c2futs).then(hpx::util::unwrapped([=](std::vector<hpx::future<void>>) {
                ...
                for (dir_type<Ndim> si; !si.is_end(); si++) {
                    if (neighbors[si] != hpx::invalid_id && !si.is_zero()) {
                        hpx::id_type neighbor = neighbors[si];
                        child_array_type local_children(children);

                        hpx::util::scoped_unlock<scoped_lock> ul(this_lock);
                        futs[si] = hpx::async<action_notify_branch>(neighbor, si, std::move(local_children));
                    } else {
    ...
    void note_sibs(std::vector<hpx::id_type> xtet) {
        //		printf("SIBLINGS %i\n", hpx::get_locality_id());
        boost::unique_lock<hpx::lcos::local::spinlock> this_lock(branch_lock);
            ...
            for (child_index_type<Ndim> ci; !ci.is_end(); ci++) {
                if (child_is_niece_of(ci, dir)) {
                    ...
                    for (dir_type<Ndim> dir2; !dir2.is_end(); dir2++) {
                        ...
                        if (use) {
                            hpx::id_type child = children[ci];
                            hpx::id_type sibling = get_sibling_of_child(ci, dir2);

                            hpx::util::scoped_unlock<scoped_lock> ul(this_lock);
                            futs.push_back(
                                    hpx::async<action_notify_of_neighbor>(
                                        child, dir2, sibling));
                        }
                    }
                } else {
    ...
    hpx::future<void> debranch() {
        ...
        for (child_index_type<Ndim> ci; !ci.is_end(); ci++) {
            if (children[ci] != hpx::invalid_id) {
                ...
                hpx::util::scoped_unlock<scoped_lock> ul(this_lock);
                cfutures[ci] = hpx::async<action_debranch>(child);
            } else {
...
//		subcycle_lock.unlock();
    ...
    void exchange_set(const dir_type<Ndim>& dir, typename Set::type data) {
        ...
        boost::unique_lock<hpx::lcos::local::spinlock> this_lock(set_lock);
